...
namespace SeitonSystem.src.dao
{
    public class FinançasDAO
    {
        ...
        public const string atualizarFluxo = "UPDATE fluxo_de_caixa SET titulo=@titulo, valor=@valor,descricao=@descricao, " +
            "data_lancamento=@data_lancamento, tipo_fluxo=@tipo_fluxo  WHERE id=@id"; // ok
        ...
        public const string selectPorData = "SELECT* FROM fluxo_de_caixa WHERE " +
            "data_lancamento Between @data1 and @data2";
        ...
        public List<Finanças> ListarPorData(string data1,string data2) // select por nome do produto
        {
            List<Finanças> lista2 = new List<Finanças>();// crio um objeto do tipo lista da minha classe
            try
            {
                 
                cmd = new MySqlCommand(selectPorData, conn); //string query para consultar produtos
